...
function Navbar() {
  ...
  const activeClass = 'bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 font-semibold transition';
  const inactiveClass = 'text-gray-700 hover:text-red-600 px-4 py-2 rounded font-semibold transition';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="text-2xl font-bold text-red-600 hover:text-red-700 transition">🍕 PizzaShop</Link>
          </div>
          <div className="flex items-center gap-4">
            ...
            {isLoggedIn ? (
              ...
              (<>
                <Link
                  to="/login"
                  className={`px-4 py-2 text-sm font-medium rounded transition ${location.pathname === '/login' ? 'bg-red-100 text-red-700' : 'text-gray-700 hover:text-red-600'}`}
                >
                  Login
                </Link>
                <Link
                  to="/signup"
                  className={`px-4 py-2 text-sm font-medium rounded transition ${location.pathname === '/signup' ? 'bg-red-600 text-white' : 'bg-red-500 text-white hover:bg-red-700'}`}
                >
                  Signup
                </Link>
              </>
